...
class Scheduler(object):
    ...
    def option(self, interval=1, name=None, notice_fun=None):
        """
        ...
        """
        job = Job(interval, self, name=name, notice_fun=notice_fun)
        return job
...
class Job(object):
    ...
    * a quantity of `time units` defined by `interval`
    ...
    """
    def __init__(self, job_func, name, unit, interval=None, at_time=None, notice_fun=None):
    ...
    def run(self):
        ...
        try:
            ret = self.job_func()
            self.notice_fun(self.name + '数据成功！')
        except:
            ret = None
            self.notice_fun(self.name+'数据失败！')
    ...
    def _schedule_next_run(self):
        ...
        if self.last_run == None:#and self.at_time != None:
            # 这里主要是为了使用任务在创建时立即运行一次。
...
def option(interval=1,name=None,notice_fun=None):
    """Calls :meth:`option <Scheduler.option>` on the
    :data:`default scheduler instance <default_scheduler>`.
    """
    return default_scheduler.option(interval,name,notice_fun)
